...
func TestNewDeflateReader(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			"success with valid zlib data",
			on{body: io.NopCloser(bytes.NewReader(compressedData))},
			want{err: nil},
		},
		{
			"error with invalid zlib data",
			on{body: io.NopCloser(bytes.NewReader([]byte("invalid data")))},
			want{err: zlib.ErrHeader},
		},
	}

	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			reader, err := NewDeflateReader(tc.on.body)
...
func TestDeflateReader_Read(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			reader, err := NewDeflateReader(io.NopCloser(bytes.NewReader(compressedData)))
			require.NoError(t, err)
			defer reader.Close()
...
func TestDeflateReader_Close(t *testing.T) {
